...
class CurriculumStrand(Base):
    ...
    strand_number: Mapped[int] = mapped_column(
        ...
        unique=True,
        nullable=False,
        comment="Strand number (1-5)"
    )
    name: Mapped[str] = mapped_column(
        String(100),
        nullable=False,
        comment="Strand name (e.g., 'Number', 'Algebra')"
    )
    color_hex: Mapped[str | None] = mapped_column(
        String(7),
        nullable=True,
        comment="UI color code (#RRGGBB)"
    )
...
class CurriculumSubStrand(Base):
    ...
    __table_args__ = (
        UniqueConstraint("strand_id", "sub_strand_number", "phase"),
    )

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    strand_id: Mapped[int] = mapped_column(
        ForeignKey("curriculum_strands.id"),
        nullable=False
    )
    sub_strand_number: Mapped[int] = mapped_column(
        SmallInteger,
        nullable=False,
        comment="Sub-strand number within strand"
    )
    phase: Mapped[str] = mapped_column(
        String(10),
        nullable=False,
        comment="Phase: 'B1_B3', 'B4_B6', 'B7_B9'"
    )
...
class CurriculumNode(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    code: Mapped[str] = mapped_column(
        ...
        unique=True,
        nullable=False,
        comment="Node code (e.g., 'B2.1.1.1')"
    )
    grade: Mapped[str] = mapped_column(
        String(5),
        nullable=False,
        comment="Grade level ('B1' through 'B9')"
    )
    ...
    sub_strand_id: Mapped[int] = mapped_column(
        ForeignKey("curriculum_sub_strands.id"),
        nullable=False
    )
    content_standard_number: Mapped[int] = mapped_column(
        SmallInteger,
        nullable=False
    )
    ...
    description: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="Full description of mastery"
    )

    severity: Mapped[int] = mapped_column(
        SmallInteger,
        nullable=False,
        comment="Severity rating (1-5, 5=most critical)"
    )
    severity_rationale: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
        comment="Why this severity rating"
    )
    ...
    questions_required: Mapped[int] = mapped_column(
        SmallInteger,
        default=2,
        comment="Min questions to confirm mastery/gap"
    )
    confidence_threshold: Mapped[float] = mapped_column(
        default=0.80,
        comment="Confidence threshold for diagnosis"
    )
    ...
    ghana_evidence: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
        comment="EGMA/NEA data, research citations"
    )
...
class CurriculumPrerequisite(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "relationship IN ('requires', 'strengthens', 'enables')",
    ...
    weight: Mapped[float] = mapped_column(
        default=1.0,
        comment="Edge weight for path analysis"
    )
...
class CurriculumIndicator(Base, UUIDPrimaryKeyMixin):
    ...
    diagnostic_prompt_example: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
        comment="Example diagnostic question"
    )
...
class IndicatorErrorPattern(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "severity IN ('critical', 'standard', 'minor')",
            name="check_error_severity"
        ),
    )
...
class CurriculumMisconception(Base):
    ...
    description: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="What the misconception IS"
    )
    evidence: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="How it manifests (observable behavior)"
    )
    root_cause: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="WHY this misconception forms"
    )
    remediation_approach: Mapped[str] = mapped_column(
        Text,
        nullable=False,
    ...
    frequency_estimate: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="e.g., '55% of students'"
    )
    source_citation: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
        comment="Research source"
    )
...
class CascadePath(Base):
    ...
    frequency: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
        comment="e.g., 'Affects ~55% of students'"
    )
