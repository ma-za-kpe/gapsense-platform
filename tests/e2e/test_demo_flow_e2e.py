...
class TestDemoFlowE2E:
    ...
    async def test_remediation_exercises_generated(self, e2e_client: AsyncClient):
        """Test that remediation exercises are generated after exercise book analysis.

        Verifies:
        1. Exercises are generated and stored in GapProfile.analysis_metadata
        2. Each exercise has required fields (question, expected_answer, teacher_note, gap_node_code)
        3. Exercises are returned in the API response
        4. Exercise count appears in dashboard
        """
        from gapsense.core.database import AsyncSessionLocal
        ...
        # Onboarding: Create teacher
        print(f"\n📍 Creating demo teacher {demo_phone}")
        resp = await e2e_client.post("/demo/api/message", data={"message": "hi", "teacher_phone": demo_phone})
        assert resp.status_code == 200
        resp = await e2e_client.post("/demo/api/message", data={"message": "Test School", "teacher_phone": demo_phone})
        assert resp.status_code == 200
        resp = await e2e_client.post("/demo/api/message", data={"message": "JHS 1", "teacher_phone": demo_phone})
        assert resp.status_code == 200

        # Onboarding: Create student
        print("📍 Creating student")
        resp = await e2e_client.post("/demo/api/message", data={"message": "1", "teacher_phone": demo_phone})
        assert resp.status_code == 200
        resp = await e2e_client.post("/demo/api/message", data={"message": "Remediation Test Student", "teacher_phone": demo_phone})
        assert resp.status_code == 200
        resp = await e2e_client.post(
            "/demo/api/message",
            data={"message": "Yes, create students", "teacher_phone": demo_phone, "button_id": "confirm_yes"},
        )
        assert resp.status_code == 200
        await asyncio.sleep(1)
        ...
        async with AsyncSessionLocal() as session:
            ...
            result = await session.execute(select(Student).where(Student.teacher_id == teacher.id))
            student = result.scalar_one()
            student_id = student.id
            print(f"✅ Student created: {student_id}")

        # Upload exercise book image
        print("📍 Uploading exercise book image")
        test_image_path = Path(__file__).parent.parent.parent / "mathhomeworkjosh.webp"
        if not test_image_path.exists():
            pytest.fail(f"Test image not found: {test_image_path}")
        ...
        # Select student to trigger analysis
        print("📍 Selecting student to trigger analysis")
        resp = await e2e_client.post("/demo/api/message", data={"message": "1", "teacher_phone": demo_phone})
        assert resp.status_code == 200

        # Wait for worker to process and generate exercises
        print("📍 Waiting for worker to process (up to 120s)")
        timeout = 120
        gap_profile = None
        ...
        for i in range(timeout):
            ...
            if gap_profile and gap_profile.analysis_metadata and "remediation_exercises" in gap_profile.analysis_metadata:
                print(f"✅ GapProfile with exercises created after {i+1} seconds")
                break

            if (i + 1) % 10 == 0:
                print(f"⏳ Waiting... {i+1}s elapsed")

        # Verify GapProfile exists
        assert gap_profile is not None, f"Worker did not create GapProfile within {timeout} seconds"
        print(f"✅ GapProfile verified: {gap_profile.id}")

        # Verify remediation exercises exist
        assert gap_profile.analysis_metadata is not None, "analysis_metadata should not be None"
        assert "remediation_exercises" in gap_profile.analysis_metadata, "remediation_exercises key missing"

        exercises = gap_profile.analysis_metadata["remediation_exercises"]
        assert isinstance(exercises, list), "remediation_exercises should be a list"
        print(f"✅ Remediation exercises found: {len(exercises)} exercises")

        # If exercises were generated, verify schema
        if len(exercises) > 0:
            for i, exercise in enumerate(exercises):
                assert isinstance(exercise, dict), f"Exercise {i} should be a dict"
                assert "question" in exercise, f"Exercise {i} missing 'question'"
                assert "expected_answer" in exercise, f"Exercise {i} missing 'expected_answer'"
                assert "teacher_note" in exercise, f"Exercise {i} missing 'teacher_note'"
                assert "gap_node_code" in exercise, f"Exercise {i} missing 'gap_node_code'"

                # Verify non-empty strings
                assert exercise["question"].strip(), f"Exercise {i} question is empty"
                assert exercise["expected_answer"].strip(), f"Exercise {i} expected_answer is empty"
                assert exercise["teacher_note"].strip(), f"Exercise {i} teacher_note is empty"
                assert exercise["gap_node_code"].strip(), f"Exercise {i} gap_node_code is empty"

            print(f"✅ All {len(exercises)} exercises have valid schema")
            print(f"   Sample exercise: {exercises[0]['question'][:80]}...")
        else:
            print("⚠️  No exercises generated (may be due to no gap nodes or AI failure - acceptable per fail-open design)")

        # Verify exercises appear in API response
        print("📍 Verifying API returns exercises")
        resp = await e2e_client.get(f"/api/reports/{demo_phone}/student/{student_id}")
        assert resp.status_code == 200
        api_data = resp.json()
        assert "remediation_exercises" in api_data, "API response missing remediation_exercises"
        assert api_data["remediation_exercises"] == exercises, "API exercises should match DB exercises"
        print(f"✅ API returns {len(api_data['remediation_exercises'])} exercises")

        # Verify exercise count in dashboard
        print("📍 Verifying dashboard shows exercise count")
        resp = await e2e_client.get(f"/api/reports/{demo_phone}")
        assert resp.status_code == 200
        dashboard_data = resp.json()

        if "latest_analysis" in dashboard_data:
            latest = dashboard_data["latest_analysis"]
            if "remediation_exercise_count" in latest:
                assert latest["remediation_exercise_count"] == len(exercises), "Dashboard count should match actual count"
                print(f"✅ Dashboard shows {latest['remediation_exercise_count']} exercises")

        print("\n🎉 Remediation Exercise E2E Test Complete!")
